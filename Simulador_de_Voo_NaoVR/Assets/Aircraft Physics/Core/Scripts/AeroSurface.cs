...
public class AeroSurface : MonoBehaviour
{
    ...
    private bool toggleGroundEffect = true;

    private float flapAngle;

    public void SetGroundEffect()
    {
        toggleGroundEffect = !toggleGroundEffect;
        config.GroundEffect = toggleGroundEffect;
    }
                                                  ...
                                                  float zeroLiftAoA)
    {
        ...
        if (h > 0.0001f && config.GroundEffect)
        {
            ...
            //liftRatio = liftRatio * 10;
        }

        //Debug.Log(config.name+":Lift: " + liftRatio.ToString()+"; h: "+h.ToString());
        //if(config.name.Contains("Wing"))
        //{
        //    Debug.Log(config.name+":Lift: " + config.GroundEffect.ToString());
        //}
